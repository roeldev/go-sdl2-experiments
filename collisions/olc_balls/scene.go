...
func newScene(stage *sdlkit.Stage) (sdlkit.Scene, error) {
	...
	// sc.mouse.OnBtnPressed(sdl.BUTTON_LEFT, func() {
	//
	// })
	// sc.mouse.OnBtnReleased(sdl.BUTTON_LEFT, func() {
	//
	// })
...
func (sc *scene) SceneName() string { return "olc_balls" }
	...
	if sc.selectedBall == nil {
		if sc.mouse.BtnLeft.Pressed && sc.mouse.BtnLeft.Clicks == 2 {
			...
			for _, ball := range sc.balls {
				if !physics.HitTestCircle(sc.mouse.X, sc.mouse.Y, ball.Circle) {
					continue
				}
...
func (sc *scene) Update(dt float64) {
	...
	sc.collisions = sc.collisions[:0]

	for i, b := range sc.balls {
		for j, t := range sc.balls {
			if i == j {
				// balls are the same, no need to check for collision
				continue
			}
			...
			t.X += overlap * x / distance
			t.Y += overlap * y / distance

			sc.collisions = append(sc.collisions, [2]*ball{b, t})
		}
	}
