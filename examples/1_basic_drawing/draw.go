...
func drawCircle(rad, x, y int32, color color.RGBA) sdlkit.Drawable {
	col := sdl.Color(color)
...
func drawRect(w, h, x, y int32, color color.RGBA) sdlkit.Drawable {
...
func drawSquare(size, x, y int32, color color.RGBA) sdlkit.Drawable {
	return drawRect(size, size, x, y, color)
}

// this is basically a sprite
func drawImg(tx *sdl.Texture, scale float32, x, y int32, flip sdl.RendererFlip) sdlkit.Drawable {
	_, _, w, h, _ := tx.Query()
	if scale != 1 {
